...
%>
	...
	<tr>
		<td> 
		<form method="post" action="text.jsp" id='<%="t"+postList.get(i).getNum()%>'> <!-- postName별로 form 생성 form id는 postNum으로 지정하여 유니크값 으로 생성  앞에 t를 붙여주는 이유는 아래의 pageNum과 구분하기 위함-->
		<input type="hidden" name="num" value=<%="t"+postList.get(i).getNum()%>> <!-- postNum의 value를 저장할 부분을 생성하나 사이트상에 표시하지는 않음 -->
		</form>
